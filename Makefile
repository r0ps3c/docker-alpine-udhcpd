...
clean-test:
	docker rm -f udhcpd-test-* 2>/dev/null || true
	docker volume rm -f udhcpd-test-* 2>/dev/null || true
	docker network rm udhcpd-test-* 2>/dev/null || true
...
show-version: build
	@FULL_VER=$$(docker run --rm --entrypoint sh $(PKGNAME):$(TAG) -c \
	  "apk info busybox-extras 2>/dev/null | grep '^busybox-extras-[0-9]' | head -1 | sed 's/^busybox-extras-//' | sed 's/ .*//' | sed 's/-r[0-9]*$$//'"); \
